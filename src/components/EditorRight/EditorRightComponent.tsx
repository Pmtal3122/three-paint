...
export default function EditorRightComponent({ camera }: { camera: cameraInterface | null }) {
  ...
  useEffect(() => {
    ...
    if (camera && camera.cameraObject) {
      ...
      rotationXInput!.value = camera.cameraObject.rotation.x.toString();
      rotationYInput!.value = camera.cameraObject.rotation.y.toString();
      rotationZInput!.value = camera.cameraObject.rotation.z.toString();
    }
  }, [])
